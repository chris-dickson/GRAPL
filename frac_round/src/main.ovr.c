/* NOT FINISHED!!!!!!!!!!!!!!!!!! */
// This one doesn't improve the objective after beating the best overflow
...
//				memcpy( &Solution[k] , &T[index_of_one] , sizeof(T[index_of_one]) );
//				Solution[k].routed = 1;			// route them all at first
			}
...
//				best_violations = violations;
				...
				for (k = 1; k <= K; k++) {
					...
					total_cost+=(double) Solution[k].tree_cost;
					total_vias+=(double) Solution[k].vias;
				}
				objective = (double)ALPHA*total_cost + (double)BETA*total_vias;
...
//		printf("Objective Value of INTEGER solution = %e\n",ALPHA*total_cost + BETA*total_vias);
