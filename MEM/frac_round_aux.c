/*----------------------------------------------------------------*/
/* frac_round_aux.c												  */
/*		Auxilliary functions for rounding code					  */
/*----------------------------------------------------------------*/



/*-------------------------------------------------------------*/
/*	Purpose: free graph vertices allocated					   */
/*  Returns: nothing										   */
/*  Args   :												   */
/*		(PathVertex*) p : vertex to freed					   */
/*-------------------------------------------------------------*/
...
/*-------------------------------------------------------------*/
/*	Purpose:  Function to match elements for quick sort        */
/*  Returns:  integer, -1 if a1<a2, 0 if equal, 1 otherwise    */
/*  Args   :                                                   */
/*		(int*) a1,a2 : array elements of integer array         */
/*-------------------------------------------------------------*/
...
/*-------------------------------------------------------------*/
/*	Purpose: Sets an array of capacities indexed by edge	   */
/*		indices												   */
/*  Returns:	c											   */
/*  Args   :												   */
/*		(int **) c : capacity array							   */
/*		(Graph*) g : pointer to the vl graph	     		   */
...
void print_capacity_lookup(FILE *fp, Graph *g) {
	ListElmt		*e;
	PathVertex		*p,*a;
	int				px,py,pz,ax,ay,az;
...
void get_capacity_lookup(char *fn, int *c, int hc, int vc) {
	FILE	*fp;
	int		index;
	char	direction;
	...
	while ( fscanf(fp,"%d %c",&index,&direction) == 2 ) 
		if (direction == 'h')
			c[index] = hc;
		else
			c[index] = vc;
...
/*-------------------------------------------------------------*/
/*	Purpose:  Used to computed edge congestion.                */
/*  Returns:  1 if edge e(i) is in tree T(j)				   */
/*  Args   :												   */
/*		(Net*) T : Our main data array of nets				   */
/*		(int) edge_i, tree_j : indices for e(i) and T(j)	   */
/*-------------------------------------------------------------*/
...
/*		(int) edge_i, tree_j : indices for e(i) and T(j)	   */
/*	Other Notes :											   */
/*		We use a binary search to speed up this lookup.  Input */
