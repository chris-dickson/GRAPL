...
------------------------------------------------------------------------------------------*/
    ...
    printf("finished\n");
    printf("Virtual Layer Graph: V = %d, E = %d\n",MLGraph[0].vcount, MLGraph[0].ecount);				/* create a 2d grid graph from the benchmark file*/
    fprintf(Stats,"Virtual Layer Graph: V = %d, E = %d\n\n",MLGraph[0].vcount, MLGraph[0].ecount);
	...
	init_threaded_heaps(&H,&A_weights,width,height,NO_THREADS);
...
//		init_thread_args(&thread_args,NO_THREADS,H,A_weights,&MLGraph, Edge2Adj, width, height, L, K, 0);
		thread_args = (thread_args_t*) malloc(sizeof(thread_args_t)*NO_THREADS);
		for (i = 0; i < NO_THREADS; i++) {
			thread_args[i].H = H[i];
			thread_args[i].A = A_weights[i];
			...
			thread_args[i].no_jobs = thread_args[i].ub - thread_args[i].lb + 1;
		}
...
------------------------------------------------------------------------------------------*/
	...
	destroy_threaded_heaps(NO_THREADS);
