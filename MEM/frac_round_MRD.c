...
int main(int argc, char *argv[]) {
	...
	if ((argc != 4)&&(argc != 5)) {
		...
		printf("\t [time] : time limit (in minutes) for rounding\n");
		exit(1);
	}	
...
/*	printf("Alpha: ");
	...
	*/
	...
	while (!done) {
		fscanf(inStatFile,"%s",a_word);
		if (!strcmp(a_word,"Alpha")) {
			fscanf(inStatFile,"%*s %s",float_value);
			ALPHA = atoi(float_value);
			done = 1;
		}
	}
	...
	while (!done) {
		fscanf(inStatFile,"%s",a_word);
		if (!strcmp(a_word,"Beta")) {
			fscanf(inStatFile,"%*s %s",float_value);
			BETA = atoi(float_value);
			done = 1;
		}
	}
	...
	while (!done) {
		...
		if (!strncmp("Checking",a_line,strlen("Checking"))) {
			done = 1;
			continue;
		}
...
//			printf("\tno_terminals = %d\n",T[l*K + k].no_terminals);
//			for (i = 0; i < T[l*K+k].no_terminals; i++)
//				printf("\t\t(%d,%d)\n",T[l*K + k].terminals[i].x, T[l*K + k].terminals[i].y);
//			printf("\tedge_count = %d\n",T[l*K+k].edge_count);
//			printf("\tvias = %d\n",T[l*K+k].vias);
//			printf("\ttree_cost = %lf\n",T[l*K+k].tree_cost);
//			printf("\tSteiner Tree:\n");
...
//	if (inSolutionFile != NULL)
//		fclose(inSolutionFile);
//	else
//		printf("INSOLUTION FILE ERROR\n");
//
//	if (inStatFile != NULL)
//		fclose(inStatFile);
//	else
//		printf("INSTATFILE ERROR\n");
