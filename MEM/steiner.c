...
//	if (NO_TERMINALS == 2) {
	if (NO_TERMINALS == 0) {
		int			vx,vy,vz,px,py,pz,tx,ty,tz;
		AdjList		*a;
		ListElmt	*e;
		int			edge_index;
		
		*edge_count_OUT = list_size(&(sPaths[0][1])) - 1;
		
		if ((*(SteinerTree_OUT)= (int*) malloc(sizeof(int) * (*edge_count_OUT)))==NULL) {
			printf("steiner.c : SteinerTree_OUT mem allocation error\n");
			fflush(stdout);
			exit(1);
		}
		
		i = 0;
		for ( element = list_head( &sPaths[0][1] ); element != NULL; element = list_next(element) ) {
			

			sPath_vertex = (sPathData*) list_data(element);
			vx = ((CoordData*) sPath_vertex->vertex)->x;
			vy = ((CoordData*) sPath_vertex->vertex)->y;
			vz = ((CoordData*) sPath_vertex->vertex)->z;
			
			if (sPath_vertex->parent != NULL) {
				px = ((CoordData*)sPath_vertex->parent)->x;
				py = ((CoordData*)sPath_vertex->parent)->y;
				pz = ((CoordData*)sPath_vertex->parent)->z;
			}
			else {
				px = -1;
				py = -1;
		...
		list_destroy(&sPaths[0][1]);
		list_destroy(&sPaths[1][0]);
		free(sPaths[0]);
		free(sPaths[1]);
		free(sPaths);
